...
import { startDownloadTask, downloadFolderAsZip, getFileVersions, uploadFile } from "@/lib/storage";
...
//
  ...
  const observeFiles = () => {
    ...
    const subscription = client.models.File.observeQuery({
      ...
      "updatedAt", "isDirectory","versions { versionId updatedAt }" ]
    }).subscribe({
      next: async ({ items }) => {
        console.log("Hello")
        if(items.length == 0){
                ...
                <ContextMenuWrapper $x={contextMenuPosition[0]} $y={contextMenuPosition[1]}>
                  <ContextMenu>
                    ...
                    {contextFile?.versions ? (
                        <>
                          <ContextMenuItem style={{ fontWeight: "bold", cursor: "default" }}>
                            Versions
                          </ContextMenuItem>
                          {[...contextFile.versions]
                        ...
                        <ContextMenuItem style={{ fontStyle: "italic", color: "#888" }}>
                          Loading versions...
                        </ContextMenuItem>
                      )}
