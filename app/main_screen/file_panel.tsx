  ...
  function sort_files(files: Array<fileInfo>){
    ...
    const observeFiles = async () => {
      const subscription = client.models.File.observeQuery({
        filter: { projectId: { eq: projectId as string } },
      }).subscribe({
        next: async ({ items }) => {
...
>>>>>>> Stashed changes
      ...
      const fetchSubscription = async () => {
        unsubscribe = await observeFiles();
      };
        ...
        )}
        {
          contextMenu && contextMenuType=="filePanel" ? (
              <ContextMenu $x={contextMenuPosition[0]} $y={contextMenuPosition[1]}>
                <ContextMenuItem onClick={() => handleCreateFile(false, projectId as string, userId as string)}>
                  Create File
                </ContextMenuItem>
                <ContextMenuItem onClick={() => handleCreateFile(true, projectId as string, userId as string)}>
                  Create Folder
                </ContextMenuItem>
                <ContextMenuItem>
                  Open Chat
